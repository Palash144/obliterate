...
1. **Rewrite commands** — Every LLM agent hook calls `obliterate rewrite "git status"`. This module decides whether to rewrite it (`obliterate git status`) or pass it through unchanged. This is the hot path — every command the LLM runs goes through here.

2. **Analyze history** — `obliterate discover` scans past LLM sessions to find commands that *could have been* rewritten but weren't. Same classification logic, different consumer.
...
2. Short-circuit special cases — `head -20 file` → `obliterate read file --max-lines 20`, `tail -n 5 file` → `obliterate read file --tail-lines 5`. These can't go through generic prefix replacement because it would produce `obliterate read -20 file` (wrong flag position)
...
4. Apply the rewrite — find the matching rule, replace the command prefix with `obliterate <cmd>`, re-prepend the env prefix, re-append the redirect suffix

**Guards along the way:**
- `OBLITERATE_DISABLED=1` in the env prefix → skip rewrite
- `gh` with `--json`/`--jq`/`--template` → skip (structured output, obliterate would corrupt it)
- `cat` with flags other than `-n` → skip (different semantics than `obliterate read`)
- `cat`/`head`/`tail` with `>` or `>>` → skip (write operation, not a read)
- Command in `hooks.exclude_commands` config → skip

**Result**: `obliterate cargo fmt --all && obliterate cargo test 2>&1 | tail -20`. Bash handles the `&&` and `|` at execution time — each `obliterate` invocation is a separate process.

## How History Analysis Works

`obliterate discover` reads Claude Code JSONL session files. Each file contains `tool_use`/`tool_result` pairs for every command the LLM ran. The module:
...
- `obliterate_cmd` — the Obliterate command it maps to (e.g., `"obliterate cargo"`)
