...
pub fn rewrite_command(
    ...
    let has_compound = trimmed.contains("&&")
        ...
        || trimmed.contains('|')
        || trimmed.contains(" & ");
    if !has_compound && (trimmed.starts_with("obliterate ") || trimmed == "obliterate") {
...
fn rewrite_line_range(cmd: &str) -> Option<String> {
    for re in [&*HEAD_N, &*HEAD_LINES] {
        if let Some(caps) = re.captures(cmd) {
            ...
            return Some(format!("obliterate read {} --max-lines {}", file, n));
        }
    }
    ...
    for re in [
        ...
        if let Some(caps) = re.captures(cmd) {
            let n = caps.get(1)?.as_str();
            let file = caps.get(2)?.as_str();
            return Some(format!("obliterate read {} --tail-lines {}", file, n));
        }
    }
    None
}
...
fn rewrite_segment_inner(
    ...
    if let Some(parts) = parse_golangci_run_parts(cmd_part) {
        let rewritten = if parts.global_segment.is_empty() {
            format!("obliterate golangci-lint {}", parts.run_segment)
        } else {
            format!(
                "obliterate golangci-lint {} {}",
...
mod tests {
    ...
    fn test_rewrite_yadm_status() {
        assert_eq!(
            ...
            Some("obliterate git status".to_string())
        );
    }
    ...
    fn test_rewrite_git_log() {
        assert_eq!(
            ...
            Some("obliterate git log -10".into())
        );
    }
    ...
    fn test_rewrite_git_dash_c_status() {
        assert_eq!(
            ...
            Some("obliterate git -C /path/to/repo status".into())
        );
    }
    ...
    fn test_rewrite_git_dash_c_log() {
        assert_eq!(
            ...
            Some("obliterate git -C /tmp/myrepo log --oneline -5".into())
        );
    }
    ...
    fn test_rewrite_git_dash_c_diff() {
        assert_eq!(
            ...
            Some("obliterate git -C /home/user/project diff --name-only".into())
        );
    }
    ...
    fn test_rewrite_background_unsupported_right() {
        assert_eq!(
            ...
            Some("obliterate cargo test & htop".into())
        );
    }
    ...
    fn test_rewrite_tsc() {
        ...
        for command in commands {
            assert_eq!(
                ...
                Some("obliterate tsc --noEmit".into()),
                "Failed for command: {}",
                command
            );
        }
    ...
    fn test_rewrite_rg_pattern() {
        assert_eq!(
            ...
            Some("obliterate grep \"fn main\"".into())
        );
    }
    ...
    fn test_rewrite_playwright() {
        ...
        for command in commands {
            assert_eq!(
                ...
                Some("obliterate playwright test".into()),
                "Failed for command: {}",
                command
            );
        }
    ...
    fn test_rewrite_next_build() {
        ...
        for command in commands {
            assert_eq!(
                ...
                Some("obliterate next --turbo".into()),
                "Failed for command: {}",
                command
            );
        }
    ...
    fn test_rewrite_pipe_first_only() {
        ...
        assert_eq!(
            ...
            Some("obliterate git log -10 | grep feat".into())
        );
    }
    ...
    fn test_rewrite_find_no_pipe_still_rewritten() {
        ...
        assert_eq!(
            ...
            Some("obliterate find . -name '*.rs'".into())
        );
    }
    ...
    fn test_rewrite_redirect_2_gt_amp_1_with_pipe() {
        assert_eq!(
            ...
            Some("obliterate cargo test 2>&1 | head".into())
        );
    }
    ...
    fn test_rewrite_redirect_2_gt_amp_1_trailing() {
        assert_eq!(
            ...
            Some("obliterate cargo test 2>&1".into())
        );
    }
    ...
    fn test_rewrite_redirect_plain_2_devnull() {
        ...
        assert_eq!(
            ...
            Some("obliterate git status 2>/dev/null".into())
        );
    }
    ...
    fn test_rewrite_redirect_2_gt_amp_1_with_and() {
        assert_eq!(
            ...
            Some("obliterate cargo test 2>&1 && echo done".into())
        );
    }
    ...
    fn test_rewrite_redirect_amp_gt_devnull() {
        assert_eq!(
            ...
            Some("obliterate cargo test &>/dev/null".into())
        );
    }
    ...
    fn test_rewrite_redirect_double() {
        ...
        assert_eq!(
            ...
            Some("obliterate git status 2>&1 >/dev/null".into())
        );
    }
    ...
    fn test_rewrite_redirect_fd_close() {
        ...
        assert_eq!(
            ...
            Some("obliterate git status 2>&-".into())
        );
    }
    ...
    fn test_rewrite_head_lines_long_flag() {
        assert_eq!(
            ...
            Some("obliterate read src/lib.rs --max-lines 50".into())
        );
    }
    ...
    fn test_rewrite_tail_numeric_flag() {
        assert_eq!(
            ...
            Some("obliterate read src/main.rs --tail-lines 20".into())
        );
    }
    ...
    fn test_rewrite_tail_n_space_flag() {
        assert_eq!(
            ...
            Some("obliterate read src/lib.rs --tail-lines 12".into())
        );
    }
    ...
    fn test_rewrite_tail_lines_long_flag() {
        assert_eq!(
            ...
            Some("obliterate read src/lib.rs --tail-lines 7".into())
        );
    }
    ...
    fn test_rewrite_glab_mr_list() {
        assert_eq!(
            ...
            Some("obliterate glab mr list".into())
        );
    }
    ...
    fn test_rewrite_glab_ci_status() {
        assert_eq!(
            ...
            Some("obliterate glab ci status".into())
        );
    }
    ...
    fn test_rewrite_diff() {
        assert_eq!(
            ...
            Some("obliterate diff file1.txt file2.txt".into())
        );
    }
    ...
    fn test_rewrite_gh_release() {
        assert_eq!(
            ...
            Some("obliterate gh release list".into())
        );
    }
    ...
    fn test_rewrite_kubectl_describe() {
        assert_eq!(
            ...
            Some("obliterate kubectl describe pod mypod".into())
        );
    }
    ...
    fn test_rewrite_docker_run() {
        assert_eq!(
            ...
            Some("obliterate docker run --rm ubuntu bash".into())
        );
    }
    ...
    fn test_rewrite_swift_test() {
        assert_eq!(
            ...
            Some("obliterate swift test --parallel".into())
        );
    }
    ...
    fn test_rewrite_docker_compose_ps() {
        assert_eq!(
            ...
            Some("obliterate docker compose ps".into())
        );
    }
    ...
    fn test_rewrite_docker_compose_logs() {
        assert_eq!(
            ...
            Some("obliterate docker compose logs web".into())
        );
    }
    ...
    fn test_rewrite_docker_compose_build() {
        assert_eq!(
            ...
            Some("obliterate docker compose build".into())
        );
    }
    ...
    fn test_rewrite_aws() {
        assert_eq!(
            ...
            Some("obliterate aws s3 ls".into())
        );
    }
    ...
    fn test_rewrite_aws_ec2() {
        assert_eq!(
            ...
            Some("obliterate aws ec2 describe-instances --region us-east-1".into())
        );
    }
    ...
    fn test_rewrite_psql() {
        assert_eq!(
            ...
            Some("obliterate psql -U postgres -d mydb".into())
        );
    }
    ...
    fn test_rewrite_ruff_check() {
        assert_eq!(
            ...
            Some("obliterate ruff check .".into())
        );
    }
    ...
    fn test_rewrite_ruff_format() {
        assert_eq!(
            ...
            Some("obliterate ruff format src/".into())
        );
    }
    ...
    fn test_rewrite_pytest() {
        assert_eq!(
            ...
            Some("obliterate pytest tests/".into())
        );
    }
    ...
    fn test_rewrite_python_m_pytest() {
        assert_eq!(
            ...
            Some("obliterate pytest -x tests/".into())
        );
    }
    ...
    fn test_rewrite_pip_outdated() {
        assert_eq!(
            ...
            Some("obliterate pip outdated".into())
        );
    }
    ...
    fn test_rewrite_golangci_lint() {
        assert_eq!(
            ...
            Some("obliterate golangci-lint run ./...".into())
        );
    }
    ...
    fn test_rewrite_golangci_lint_with_flag_before_run() {
        assert_eq!(
            ...
            Some("obliterate golangci-lint -v run ./...".into())
        );
    }
    ...
    fn test_rewrite_golangci_lint_with_value_flag_before_run() {
        assert_eq!(
            ...
            Some("obliterate golangci-lint --color never run ./...".into())
        );
    }
    ...
    fn test_rewrite_golangci_lint_with_inline_config_flag_before_run() {
        assert_eq!(
            ...
            Some("obliterate golangci-lint --config=foo.yml run ./...".into())
        );
    }
    ...
    fn test_rewrite_prisma() {
        ...
        for command in commands {
            assert_eq!(
                ...
                Some("obliterate prisma migrate dev".into()),
                "Failed for command: {}",
                command
            );
        }
    ...
    fn test_rewrite_prettier() {
        ...
        for command in commands {
            assert_eq!(
                ...
                Some("obliterate prettier --check src/".into()),
                "Failed for command: {}",
                command
            );
        }
    ...
    fn test_rewrite_pnpm_command() {
        ...
        for command in commands {
            assert_eq!(
                ...
                Some(format!("obliterate pnpm {command}")),
    ...
    fn test_rewrite_npm_bare_subcommand() {
        ...
        for command in commands {
            assert_eq!(
                ...
                Some(format!("obliterate npm {command}")),
    ...
    fn test_rewrite_npm_with_args() {
        assert_eq!(
            ...
            Some("obliterate npm run test".to_string()),
        );
        assert_eq!(
            rewrite_command_no_prefixes("npm exec vitest", &[]),
            Some("obliterate vitest".to_string()),
        );
    }
    ...
    fn test_rewrite_npx() {
        assert_eq!(
            ...
            Some("obliterate npx svgo".to_string()),
        );
    }
    ...
    fn test_rewrite_gradlew() {
        assert_eq!(
            ...
            Some("obliterate gradlew assembleDebug".into())
        );
    }
    ...
    fn test_rewrite_gradlew_no_dot_slash() {
        assert_eq!(
            ...
            Some("obliterate gradlew build".into())
        );
    }
    ...
    fn test_rewrite_gradlew_bat() {
        assert_eq!(
            ...
            Some("obliterate gradlew clean".into())
        );
    }
    ...
    fn test_rewrite_compound_pipe_raw_filter() {
        ...
        assert_eq!(
            ...
            Some("obliterate cargo test | grep FAILED".into())
        );
    }
    ...
    fn test_rewrite_find_with_flags() {
        assert_eq!(
            ...
            Some("obliterate find . -name '*.rs' -type f".into())
        );
    }
    ...
    fn test_rewrite_compound_partial_exclude() {
        ...
        assert_eq!(
            ...
            Some("obliterate git status && curl https://api.example.com".into())
        );
    }
    ...
    fn test_rewrite_gh_without_json_still_works() {
        assert_eq!(
            ...
            Some("obliterate gh pr list".into())
        );
    }
    ...
    fn test_rewrite_git_dash_c() {
        assert_eq!(
            ...
            Some("obliterate git -C /tmp status".to_string())
        );
    }
    ...
    fn test_rewrite_git_no_pager() {
        assert_eq!(
            ...
            Some("obliterate git --no-pager log -5".to_string())
        );
    }
    ...
    fn test_rewrite_wc() {
        assert_eq!(
            ...
            Some("obliterate wc -l src/main.rs".into())
        );
    }
    ...
    fn test_rewrite_wc_multi_file() {
        assert_eq!(
            ...
            Some("obliterate wc src/*.rs".into())
        );
    }
    ...
    fn test_rewrite_command_substitution_passthrough() {
        assert_eq!(
            ...
            Some("obliterate git log $(git rev-parse HEAD~1)".into())
        );
    }
    ...
    fn test_pip_show() {
        assert_eq!(
            ...
            Some("obliterate pip show flask".into())
        );
    }
