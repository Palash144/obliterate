...
/// Hook status for diagnostics and `obliterate gain`.
...
pub fn status() -> HookStatus {
    ...
    if binary_hook_registered(&claude_dir) {
        ...
        // (migration not complete — user should run `obliterate init -g` to clean up)
...
fn check_and_warn() -> Option<()> {
    let warning = match status() {
        ...
        HookStatus::Missing => {
            "[obliterate] /!\\ No hook installed — run `obliterate init -g` for automatic token savings"
        }
        HookStatus::Outdated => "[obliterate] /!\\ Hook outdated — run `obliterate init -g` to update",
    };
