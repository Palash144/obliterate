...
Owns: `obliterate init` installation flows (5 agents via `AgentTarget` enum + 3 special modes: Gemini, Codex, OpenCode), SHA-256 integrity verification, hook version checking, audit log analysis, `obliterate rewrite` CLI entry point, and TOML filter trust management.
...
Boundary notes:
- `rewrite_cmd.rs` is a thin CLI bridge — it exists to serve hooks (hooks call `obliterate rewrite` as a subprocess) and delegates entirely to `discover/registry`.
...
## Purpose
LLM agent integration layer that installs, validates, and executes command-rewriting hooks for AI coding assistants. Hooks intercept raw CLI commands (e.g., `git status`) and rewrite them to Obliterate equivalents (e.g., `obliterate git status`) so that LLM agents automatically benefit from token savings without explicit user configuration.

## Installation Modes

`obliterate init` supports these installation flows:

| Mode | Command | Creates | Patches |
|------|---------|---------|----------|
| Default (global) | `obliterate init -g` | Hook, SHA-256 hash, Obliterate.md | settings.json, CLAUDE.md |
| Hook only | `obliterate init -g --hook-only` | Hook, SHA-256 hash | settings.json |
| Claude-MD (legacy) | `obliterate init --claude-md` | 134-line Obliterate block | CLAUDE.md |
| Windsurf | `obliterate init -g --agent windsurf` | `.windsurfrules` | -- |
| Cline | `obliterate init --agent cline` | `.clinerules` | -- |
| Codex | `obliterate init --codex` | Obliterate.md in `$CODEX_HOME` or `~/.codex` | AGENTS.md |
| Cursor | `obliterate init -g --agent cursor` | Cursor hook | hooks.json |
| Pi | `obliterate init --agent pi` | `.pi/extensions/obliterate.ts` | -- |
| Hermes | `obliterate init --agent hermes` | Python plugin in `~/.hermes/plugins/obliterate-rewrite/` | `config.yaml` `plugins.enabled` |
...
1. At install: `integrity::store_hash()` computes SHA-256 of the hook file, writes to `~/.claude/hooks/.obliterate-hook.sha256` (read-only 0o444)
2. At runtime: `integrity::runtime_check()` re-computes hash and compares; blocks execution if tampered
3. On demand: `obliterate verify` prints detailed verification status (PASS/FAIL/WARN/SKIP)
...
Controls how `obliterate init` modifies agent settings files:
...
All file operations use atomic writes (tempfile + rename) to prevent corruption on crash. Settings files are backed up to `.bak` before modification. All operations are idempotent -- running `obliterate init` multiple times is safe.

## Permission Model

Obliterate enforces a permission precedence that matches Claude Code's least-privilege default:
...
|------|------------|-------------------|
| Claude Code (obliterate-rewrite.sh) | Yes | `permissionDecision: "ask"` — user prompted |
| Copilot VS Code (obliterate hook copilot) | Yes | `permissionDecision: "ask"` — user prompted |
| Gemini CLI (obliterate hook gemini) | No (allow/deny only) | allow (limitation — no ask mode in Gemini) |
| Copilot CLI (obliterate hook copilot) | No updatedInput | deny-with-suggestion (unchanged) |
...
To add support for a new AI coding agent: (1) add the hook installation logic to `init.rs` following the existing agent patterns, (2) if the agent requires a custom hook protocol (like Gemini's `BeforeTool`), add a processor function in `hook_cmd.rs`, (3) add the agent's hook file path to `hook_check.rs` for validation, and (4) update `integrity.rs` with the expected hash for the new hook file. Test by running `obliterate init` in a fresh environment and verifying the hook rewrites commands correctly in the target agent.
