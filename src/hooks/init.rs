...
/// Used when removing @OBLITERATE.md line from CLAUDE.md
...
fn run_default_mode(
    ...
    // 3. Patch CLAUDE.md (add @OBLITERATE.md, migrate if needed)
...
/// Hook-only mode: just the hook, no OBLITERATE.md
...
/// Patch CLAUDE.md: add @OBLITERATE.md, migrate if old block exists
fn patch_claude_md(path: &Path, ctx: InitContext) -> Result<bool> {
    ...
    if dry_run {
        println!(
            "[dry-run] would add @OBLITERATE.md reference to CLAUDE.md: {}",
            path.display()
        );
        if verbose > 0 {
            println!("[dry-run] content:\n{}", new_content);
        ...
        if verbose > 0 {
            eprintln!("Added @OBLITERATE.md reference to CLAUDE.md");
        }
    }
...
fn remove_obliterate_reference_from_agents(path: &Path, refs: &[&str], ctx: InitContext) -> Result<bool> {
    ...
    if dry_run {
        println!(
            "[dry-run] would remove OBLITERATE.md reference from AGENTS.md: {}",
            path.display()
        );
        if verbose > 0 {
            println!("[dry-run] content:\n{}", cleaned);
    ...
    if verbose > 0 {
        eprintln!(
            "Removed OBLITERATE.md reference from AGENTS.md: {}",
            path.display()
        );
    }
...
/// Remove Cursor Obliterate artifacts: hook script + hooks.json entry
...
fn show_claude_config() -> Result<()> {
    ...
    println!("Obliterate Configuration:\n");
    ...
    if binary_hook_registered {
        ...
        #[cfg(unix)]
        {
            ...
            if !is_executable {
                ...
                println!(
                    "[warn] Hook: {} (outdated — run `obliterate init -g` to upgrade to native binary)",
                    hook_path.display()
                );
            } else if is_executable && has_guards {
                println!(
                    "[warn] Hook: {} (legacy script v{} — run `obliterate init -g` to upgrade)",
                    hook_path.display(),
                    hook_version
                );
            } else {
        ...
        {
            println!(
                "[warn] Hook: {} (legacy script — run `obliterate init -g` to upgrade)",
                hook_path.display()
            );
        }
    } else {
        ...
        if cursor_binary_registered {
            ...
            #[cfg(unix)]
            {
                ...
                if !is_executable {
                    ...
                    println!(
                        "[warn] Cursor hook: {} (legacy script — run `obliterate init -g --agent cursor` to upgrade)",
                        cursor_hook.display()
                    );
                }
            }
            ...
            #[cfg(not(unix))]
            {
                println!("[warn] Cursor hook: {} (legacy script — run `obliterate init -g --agent cursor` to upgrade)", cursor_hook.display());
            }
        } else {
    ...
    println!("\nUsage:");
    println!("  obliterate init              # Full injection into local CLAUDE.md");
    println!("  obliterate init -g           # Hook + OBLITERATE.md + @OBLITERATE.md + settings.json (recommended)");
    println!("  obliterate init -g --auto-patch    # Same as above but no prompt");
    println!("  obliterate init -g --no-patch      # Skip settings.json (manual setup)");
    println!("  obliterate init -g --uninstall     # Remove all Obliterate artifacts");
    println!("  obliterate init -g --claude-md     # Legacy: full injection into ~/.claude/CLAUDE.md");
    println!("  obliterate init -g --hook-only     # Hook only, no OBLITERATE.md");
    println!("  obliterate init --codex            # Configure local AGENTS.md + OBLITERATE.md");
    println!("  obliterate init -g --codex         # Configure $CODEX_HOME/AGENTS.md + $CODEX_HOME/OBLITERATE.md (or ~/.codex/)");
    println!("  obliterate init -g --opencode      # OpenCode plugin only");
    println!("  obliterate init -g --agent cursor  # Install Cursor Agent hooks");
...
fn show_codex_config() -> Result<()> {
    ...
    println!("  obliterate init --codex              # Configure local AGENTS.md + OBLITERATE.md");
    println!("  obliterate init -g --codex           # Configure $CODEX_HOME/AGENTS.md + $CODEX_HOME/OBLITERATE.md (or ~/.codex/)");
    println!("  obliterate init -g --codex --uninstall  # Remove global Codex Obliterate artifacts");
...
/// Entry point for `obliterate init --gemini`
pub fn run_gemini(
    global: bool,
    hook_only: bool,
    patch_mode: PatchMode,
...
const COPILOT_HOOK_JSON: &str = r#"{
  "hooks": {
    "PreToolUse": [
      {
        "type": "command",
        "command": "obliterate hook copilot",
        "cwd": ".",
        "timeout": 5
      }
    ]
...
mod tests {
    ...
    fn test_init_is_idempotent() {
        ...
        let count = content.matches("@OBLITERATE.md").count();
        assert_eq!(count, 1);
    }
    ...
    fn test_patch_agents_md_adds_reference_once() {
        ...
        assert_eq!(content.matches("@OBLITERATE.md").count(), 1);
    }
    ...
    fn test_patch_agents_md_creates_missing_file() {
        ...
        assert!(added);
        let content = fs::read_to_string(&agents_md).unwrap();
        assert_eq!(content, "@OBLITERATE.md\n");
    }
    ...
    fn test_uninstall_handles_both_artifacts() {
        ...
        let after_at_removal: String = content
            .lines()
            .filter(|line| !line.trim().starts_with("@OBLITERATE.md"))
            .collect::<Vec<_>>()
            .join("\n");
