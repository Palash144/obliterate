...
/// Parsed arguments from either native find or Obliterate find syntax.
#[derive(Debug)]
struct FindArgs {
    pattern: String,
    path: String,
...
/// Native find flags that Obliterate cannot handle correctly.
...
/// Parse arguments from raw args vec, supporting both native find and Obliterate syntax.
///
/// Native find syntax: `find . -name "*.rs" -type f -maxdepth 3`
/// Obliterate syntax: `find *.rs [path] [-m max] [-t type]`
fn parse_find_args(args: &[String]) -> Result<FindArgs> {
    ...
    if has_unsupported_find_flags(args) {
        anyhow::bail!(
            "obliterate find does not support compound predicates or actions (e.g. -not, -exec). Use `find` directly."
        );
    }
    ...
    if has_native_find_flags(args) {
        ...
        parse_obliterate_find_args(args)
    }
}
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            flag if flag.starts_with('-') => {
                eprintln!("obliterate find: unknown flag '{}', ignored", flag);
            }
            _ => {}
        }
        i += 1;
...
/// Parse Obliterate syntax: `find <pattern> [path] [-m max] [-t type]`
fn parse_obliterate_find_args(args: &[String]) -> Result<FindArgs> {
...
pub fn run(
    ...
    let obliterate_output = format!("{}F {}D + {}", total_files, dirs_count, ext_line);
    timer.track(
        &format!("find {} -name '{}'", path, effective_pattern),
        "obliterate find",
        &raw_output,
...
mod tests {
    ...
    // --- parse_find_args: Obliterate syntax ---

    #[test]
    fn parse_obliterate_syntax_pattern_only() {
    ...
    #[test]
    fn parse_obliterate_syntax_pattern_and_path() {
    ...
    #[test]
    fn parse_obliterate_syntax_with_flags() {
    ...
    #[test]
    fn run_from_args_obliterate_syntax() {
        // Simulates: obliterate find *.rs src
