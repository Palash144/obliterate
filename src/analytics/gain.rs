...
pub fn run(
    ...
    if !daily && !weekly && !monthly && !all {
        ...
        let title = if project_scope.is_some() {
            "Obliterate Token Savings (Project Scope)"
        } else {
        ...
        match hook_check::status() {
            hook_check::HookStatus::Missing => {
                eprintln!(
                    "{}",
                    "[warn] No hook installed — run `obliterate init -g` for automatic token savings"
                        .yellow()
                );
                eprintln!();
            }
            hook_check::HookStatus::Outdated => {
                eprintln!(
                    "{}",
                    "[warn] Hook outdated — run `obliterate init -g` to update".yellow()
                );
                eprintln!();
            }
