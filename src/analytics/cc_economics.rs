...
//!
//! Combines ccusage (tokens spent) with obliterate tracking (tokens saved) to provide
...
pub struct PeriodEconomics {
    ...
    // obliterate metrics
    pub obliterate_commands: Option<usize>,
    pub obliterate_saved_tokens: Option<usize>,
    pub obliterate_savings_pct: Option<f64>,
...
impl PeriodEconomics {
    fn new(label: &str) -> Self {
        Self {
            ...
            obliterate_commands: None,
            obliterate_saved_tokens: None,
            obliterate_savings_pct: None,
    ...
    fn set_obliterate_from_day(&mut self, stats: &DayStats) {
        self.obliterate_commands = Some(stats.commands);
        self.obliterate_saved_tokens = Some(stats.saved_tokens);
        self.obliterate_savings_pct = Some(stats.savings_pct);
    }

    fn set_obliterate_from_week(&mut self, stats: &WeekStats) {
        self.obliterate_commands = Some(stats.commands);
        self.obliterate_saved_tokens = Some(stats.saved_tokens);
        self.obliterate_savings_pct = Some(stats.savings_pct);
    }

    fn set_obliterate_from_month(&mut self, stats: &MonthStats) {
        self.obliterate_commands = Some(stats.commands);
        self.obliterate_saved_tokens = Some(stats.saved_tokens);
        self.obliterate_savings_pct = Some(if stats.input_tokens + stats.output_tokens > 0 {
    ...
    fn compute_weighted_metrics(&mut self) {
        ...
        if let (Some(cost), Some(saved)) = (self.cc_cost, self.obliterate_saved_tokens) {
...
struct Totals {
    ...
    obliterate_commands: usize,
    obliterate_saved_tokens: usize,
    obliterate_avg_savings_pct: f64,
...
fn merge_daily(cc: Option<Vec<CcusagePeriod>>, obliterate: Vec<DayStats>) -> Vec<PeriodEconomics> {
    ...
    // Merge obliterate data
    for entry in obliterate {
        map.entry(entry.date.clone())
            .or_insert_with_key(|k| PeriodEconomics::new(k))
            .set_obliterate_from_day(&entry);
    }
...
fn merge_weekly(cc: Option<Vec<CcusagePeriod>>, obliterate: Vec<WeekStats>) -> Vec<PeriodEconomics> {
    let mut map: HashMap<String, PeriodEconomics> = HashMap::new();
    ...
    // Merge obliterate data (week_start = legacy Saturday "2026-01-18")
    ...
    for entry in obliterate {
        ...
        map.entry(monday_key)
            ...
            .set_obliterate_from_week(&entry);
    }
...
fn merge_monthly(cc: Option<Vec<CcusagePeriod>>, obliterate: Vec<MonthStats>) -> Vec<PeriodEconomics> {
    let mut map: HashMap<String, PeriodEconomics> = HashMap::new();
    ...
    for entry in obliterate {
        map.entry(entry.month.clone())
            .or_insert_with_key(|k| PeriodEconomics::new(k))
            .set_obliterate_from_month(&entry);
    }
...
/// Convert Saturday week_start (legacy obliterate) to ISO Monday
...
fn convert_saturday_to_monday(saturday: &str) -> Option<String> {
    ...
    // obliterate uses Saturday as week start, ISO uses Monday
...
fn compute_totals(periods: &[PeriodEconomics]) -> Totals {
    let mut totals = Totals {
        ...
        obliterate_commands: 0,
        obliterate_saved_tokens: 0,
        obliterate_avg_savings_pct: 0.0,
        weighted_input_cpt: None,
        savings_weighted: None,
        blended_cpt: None,
        active_cpt: None,
    ...
    for p in periods {
        ...
        if let Some(cmds) = p.obliterate_commands {
            totals.obliterate_commands += cmds;
        }
        if let Some(saved) = p.obliterate_saved_tokens {
            totals.obliterate_saved_tokens += saved;
        }
        if let Some(pct) = p.obliterate_savings_pct {
            pct_sum += pct;
            pct_count += 1;
        }
    }
    ...
    if pct_count > 0 {
        totals.obliterate_avg_savings_pct = pct_sum / pct_count as f64;
    }
    ...
    if weighted_units > 0.0 {
        ...
        totals.savings_weighted = Some(totals.obliterate_saved_tokens as f64 * input_cpt);
    }
    ...
    if totals.cc_total_tokens > 0 {
        ...
        totals.savings_blended = Some(totals.obliterate_saved_tokens as f64 * totals.blended_cpt.unwrap());
    }
    if totals.cc_active_tokens > 0 {
        totals.active_cpt = Some(totals.cc_cost / totals.cc_active_tokens as f64);
        totals.savings_active = Some(totals.obliterate_saved_tokens as f64 * totals.active_cpt.unwrap());
    }
...
fn display_summary(tracker: &Tracker, verbose: u8) -> Result<()> {
    ...
    let obliterate_monthly = tracker
        .get_by_month()
        .context("Failed to load monthly token savings from database")?;
    let periods = merge_monthly(cc_monthly, obliterate_monthly);

    if periods.is_empty() {
        println!("No data available. Run some obliterate commands to start tracking.");
        return Ok(());
    }
    ...
    println!("  Obliterate commands:                 {}", totals.obliterate_commands);
    println!(
        "  Tokens saved:                 {}",
        format_tokens(totals.obliterate_saved_tokens)
    );
    println!();
    ...
    println!("  Obliterate compresses CLI outputs before they enter Claude's context.");
...
fn display_daily(tracker: &Tracker, verbose: u8) -> Result<()> {
    ...
    let obliterate_daily = tracker
        .get_all_days()
        .context("Failed to load daily token savings from database")?;
    let periods = merge_daily(cc_daily, obliterate_daily);
...
fn display_weekly(tracker: &Tracker, verbose: u8) -> Result<()> {
    ...
    let obliterate_weekly = tracker
        .get_by_week()
        .context("Failed to load weekly token savings from database")?;
    let periods = merge_weekly(cc_weekly, obliterate_weekly);
...
fn print_period_table(periods: &[PeriodEconomics], verbose: u8) {
    ...
    if verbose > 0 {
        ...
        println!(
            ...
            "Period", "Spent", "Saved", "Savings", "Active$", "Blended$", "Obliterate Cmds"
        );
        println!(
            "{:-<12} {:-<10} {:-<10} {:-<10} {:-<10} {:-<12} {:-<12}",
            "", "", "", "", "", "", ""
        ...
        println!(
            "{:<12} {:>10} {:>10} {:>10} {:>12}",
            "Period", "Spent", "Saved", "Savings", "Obliterate Cmds"
        );
        println!(
            "{:-<12} {:-<10} {:-<10} {:-<10} {:-<12}",
            "", "", "", "", ""
...
fn export_json(
    ...
    if all || daily {
        ...
        let obliterate = tracker
            .get_all_days()
            .context("Failed to load daily token savings for JSON export")?;
        export.daily = Some(merge_daily(cc, obliterate));
    }
    ...
    if all || weekly {
        ...
        export.weekly = Some(merge_weekly(cc, obliterate));
    }
    ...
    if all || monthly {
        ...
        let periods = merge_monthly(cc, obliterate);
...
fn export_csv(
    ...
    println!("period,spent,input_tokens,output_tokens,cache_create,cache_read,active_tokens,total_tokens,saved_tokens,weighted_savings,active_savings,blended_savings,obliterate_commands");
    ...
    if all || daily {
        ...
        let obliterate = tracker
            .get_all_days()
            .context("Failed to load daily token savings for JSON export")?;
        let periods = merge_daily(cc, obliterate);
        for p in periods {
            print_csv_row(&p);
        }
    }
    ...
    if all || weekly {
        ...
        let obliterate = tracker
            .get_by_week()
            .context("Failed to load weekly token savings for export")?;
        let periods = merge_weekly(cc, obliterate);
        for p in periods {
            print_csv_row(&p);
        }
    }
...
fn print_csv_row(p: &PeriodEconomics) {
    ...
    let blended_savings = p
        ...
        .map(|s| format!("{:.4}", s))
        .unwrap_or_default();
    let cmds = p.obliterate_commands.map(|c| c.to_string()).unwrap_or_default();
...
mod tests {
    ...
    fn test_period_economics_new() {
        ...
        assert!(p.obliterate_commands.is_none());
    }
    ...
    fn test_compute_dual_metrics_with_data() {
        let mut p = PeriodEconomics {
            ...
            obliterate_saved_tokens: Some(5_000),
    ...
    fn test_merge_monthly_both_present() {
        ...
        let merged = merge_monthly(Some(cc), obliterate);
        assert_eq!(merged.len(), 1);
        assert_eq!(merged[0].label, "2026-01");
        assert_eq!(merged[0].cc_cost, Some(12.34));
        assert_eq!(merged[0].obliterate_commands, Some(10));
    }
    ...
    fn test_merge_monthly_only_ccusage() {
        ...
        assert_eq!(merged.len(), 1);
        assert_eq!(merged[0].cc_cost, Some(12.34));
        assert!(merged[0].obliterate_commands.is_none());
    }

    #[test]
    fn test_merge_monthly_only_obliterate() {
        let obliterate = vec![MonthStats {
            month: "2026-01".to_string(),
            commands: 10,
            input_tokens: 800,
        ...
        let merged = merge_monthly(None, obliterate);
        assert_eq!(merged.len(), 1);
    ...
    fn test_compute_weighted_input_cpt() {
        ...
        p.obliterate_saved_tokens = Some(10_000);
    ...
    fn test_compute_weighted_metrics_zero_tokens() {
        ...
        p.obliterate_saved_tokens = Some(5000);
    ...
    fn test_compute_weighted_metrics_no_cache() {
        ...
        p.cc_cache_create_tokens = Some(0);
        p.cc_cache_read_tokens = Some(0);
        p.obliterate_saved_tokens = Some(3000);
    ...
    fn test_compute_totals() {
        let periods = vec![
            PeriodEconomics {
                ...
                obliterate_commands: Some(5),
                obliterate_saved_tokens: Some(2000),
                obliterate_savings_pct: Some(50.0),
                weighted_input_cpt: None,
                savings_weighted: None,
                blended_cpt: None,
                active_cpt: None,
            ...
            PeriodEconomics {
                ...
                obliterate_commands: Some(10),
                obliterate_saved_tokens: Some(3000),
                obliterate_savings_pct: Some(60.0),
                weighted_input_cpt: None,
                savings_weighted: None,
                blended_cpt: None,
                active_cpt: None,
        ...
        assert_eq!(totals.obliterate_commands, 15);
        assert_eq!(totals.obliterate_saved_tokens, 5000);
        assert_eq!(totals.obliterate_avg_savings_pct, 55.0);
