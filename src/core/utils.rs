...
//!
//! Provides common helpers used across obliterate commands:
...
/// # Examples
/// ```
/// use obliterate::utils::strip_ansi;
...
/// # Examples
/// ```
/// use obliterate::utils::format_tokens;
...
/// # Examples
/// ```
/// use obliterate::utils::format_usd;
...
/// # Examples
/// ```
/// use obliterate::utils::format_cpt;
...
/// # Examples
/// ```
/// use obliterate::utils::join_with_overflow;
...
/// # Examples
/// ```
/// use obliterate::utils::truncate_iso_date;
...
/// # Examples
/// ```
/// use obliterate::utils::ok_confirmation;
        ...
        None => {
            #[cfg(unix)]
            {
                ...
                if let Some(sig) = output.status.signal() {
                    eprintln!("[obliterate] {}: process terminated by signal {}", label, sig);
                    return 128 + sig;
                }
            }
            eprintln!("[obliterate] {}: process terminated by signal", label);
            1
        }
    }
}
...
pub fn fallback_tail(output: &str, label: &str, n: usize) -> String {
    eprintln!(
        "[obliterate] {}: output format not recognized, showing last {} lines",
        label, n
    );
...
/// # Examples
/// ```no_run
/// use obliterate::utils::detect_package_manager;
    ...
    match resolve_binary(name) {
        ...
        Err(e) => {
            ...
            if cfg!(any(target_os = "windows", debug_assertions)) {
                eprintln!(
                    "obliterate: Failed to resolve '{}' via PATH, falling back to direct exec: {}",
                    name, e
                );
            }
...
mod tests {
    ...
    fn test_detect_package_manager_default() {
        // In the test environment (obliterate repo), there's no JS lockfile
