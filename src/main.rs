...
#[command(
    name = "obliterate",
    version,
    about = "Obliterate - Snip noisy command output before it hits your AI",
...
enum Commands {
    ...
    /// Initialize obliterate instructions for assistant CLI usage
    Init {
        ...
        /// Remove Obliterate artifacts for the selected assistant mode
        #[arg(long)]
        uninstall: bool,

        /// Target Codex CLI (uses AGENTS.md + OBLITERATE.md, no Claude hook patching)
        #[arg(long)]
        codex: bool,
    ...
    /// Discover missed Obliterate savings from Claude Code history
    Discover {
    ...
    /// Show Obliterate adoption across Claude Code sessions
    Session {},
    ...
    match command {
        PnpmCommands::Typecheck { .. } => {
            ...
            if !filters.is_empty() {
                ...
                let msg = format!(
                    "[obliterate] warning: --filter is not yet supported for pnpm {}, filters preceding the subcommand will be ignored",
                    cmd_name
                );
                return Some(msg);
            }
...
mod tests {
    ...
    fn test_rewrite_clap_multi_args() {
        // This is the bug KuSh reported: `obliterate rewrite ls -al` failed because
    ...
    fn test_pnpm_typecheck_with_filters() {
        ...
        match cli.command {
            Commands::Pnpm { filter, command } => {
                ...
                assert_eq!(filter, vec!["@app1", "@app2"]);
                assert_eq!(warning, "[obliterate] warning: --filter is not yet supported for pnpm tsc, filters preceding the subcommand will be ignored")
            }
            _ => panic!("Expected Pnpm Build command"),
        }
    }
